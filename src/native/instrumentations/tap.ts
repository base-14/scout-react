...
function getJsxRuntimes(): [any | null, any | null] {
  let jsxRt: any = null;
...
function emitTapSpan(
  ...
  let source: string;
  if (target.accessibilityLabel) source = 'standard_attribute';
  else if (target.testID) source = 'standard_attribute';
  else if (target.children) source = 'text_content';
  else if (inst) source = resolveTargetName(inst).source;
  else source = 'blank';
...
export function installNativeTapTracker(scout: Scout): () => void {
  observeTap = (callArgs: any[]) => {
    ...
    if (event) (event as any).__scoutTapRecorded = true;
  };
  ...
  g.__scoutTap = (
    ...
    const event = resolveNativeEvent(args);
    if (event && (event as any).__scoutTapRecorded) return;
    emitTapSpan(scout, target, args);
    if (event) (event as any).__scoutTapRecorded = true;
  };
  return () => {
    observeTap = null;
    if (g.__scoutTap) delete g.__scoutTap;
  };
}
function resolveNativeEvent(args: any[]): any | null {
  if (!args || args.length === 0 || !args[0]) return null;
  if (args[0]._targetInst) return args[0];
  if (args[0].event && args[0].event._targetInst) return args[0].event;
  return null;
}

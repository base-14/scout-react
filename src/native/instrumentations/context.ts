      ...
      const apply = (state: any) => {
        ...
        if (state.details?.carrier) {
          scout.setRuntimeAttribute(
            ATTR.NETWORK_CELLULAR_CARRIER_NAME,
            String(state.details.carrier),
          );
        }
      };
    ...
    const snap = async () => {
      try {
        const out: Record<string, unknown> | null =
          await ScoutCrash.getAccessibilitySnapshot();
        if (!out) return;
        ...
        for (const [native, attr] of mapping) {
          ...
          if (typeof v === 'boolean') scout.setRuntimeAttribute(attr, v);
        }
      } catch {}
    };
    void snap();

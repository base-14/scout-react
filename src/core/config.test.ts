...
describe('resolveConfig', () => {
    ...
    it('clamps sessionSampleRate to 0..100', () => {
        expect(resolveConfig({ serviceName: 's', endpoint: 'e', sessionSampleRate: -10 })
            .sessionSampleRate).toBe(0);
        expect(resolveConfig({ serviceName: 's', endpoint: 'e', sessionSampleRate: 500 })
            .sessionSampleRate).toBe(100);
        expect(resolveConfig({ serviceName: 's', endpoint: 'e', sessionSampleRate: 25 })
            .sessionSampleRate).toBe(25);
    });
...
describe('resolveEndpoint', () => {
    it('keeps a fully-qualified https URL', () => {
        expect(resolveEndpoint('https://otlp.example.com:4318', true)).toBe('https://otlp.example.com:4318');
    });
    ...
    it('prepends https when secure=true and no scheme', () => {
        expect(resolveEndpoint('otlp.example.com:4318', true)).toBe('https://otlp.example.com:4318');
    });

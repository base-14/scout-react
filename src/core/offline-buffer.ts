...
export class OfflineBuffer {
    constructor(private readonly storage: Storage, private readonly maxItemsPerSignal: Record<SignalType, number>) { }
    ...
    async drain(signal: SignalType, url: string, headers: Record<string, string>): Promise<{
        sent: number;
        remaining: number;
    }> {
        const batches = await this.readAll(signal);

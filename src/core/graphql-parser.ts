...
export function parseGraphQLRequest(body: unknown): GraphQLRequestInfo | null {
  ...
  const opMatch = query.match(
    /^\s*(query|mutation|subscription)\b\s*([A-Za-z_][A-Za-z0-9_]*)?/,
  );
  const opType = (opMatch?.[1] as OpType | undefined) ?? 'query';
  const opName: string | undefined =
    typeof obj.operationName === 'string' ? obj.operationName : opMatch?.[2];
  return {
...
export function parseGraphQLResponse(body: unknown): GraphQLResponseInfo | null {
  ...
  if (!('data' in obj) && errors.length === 0) return null;

...
export class Scout {
    ...
    async bootstrap(): Promise<void> {
        ...
        try {
            ...
            this.viewCounters.long_task = this.meter.createCounter(METRIC.VIEW_LONG_TASK_COUNT, {
                description: 'Count of long_task spans on this view',
            });
            this.viewCounters.frozen_frame = this.meter.createCounter(METRIC.VIEW_FROZEN_FRAME_COUNT, { description: 'Count of frozen_frame spans on this view' });
            ...
            this.viewCounters.frustration = this.meter.createCounter(METRIC.VIEW_FRUSTRATION_COUNT, { description: 'Count of frustration-flagged interactions on this view' });
        }
        catch {
        }
    ...
    commonAttributes(): Attributes {
        ...
        if (anon)
            attrs[ATTR.ENDUSER_ANONYMOUS_ID] = anon;
        return attrs;
    }
    ...
    private _activeVitals = new Map<string, {
        startedAt: number;
        description?: string;
    }>();
    ...
    endVital(name: string): void {
        ...
        if (v.description)
            attrs[ATTR.VITAL_DESCRIPTION] = v.description;
    ...
    recordOperationStep(name: string, stepType: 'start' | 'update' | 'retry' | 'end', opts?: {
        ...
        if (opts?.key)
            attrs[ATTR.OPERATION_KEY] = opts.key;
        if (opts?.failureReason)
            attrs[ATTR.OPERATION_FAILURE_REASON] = opts.failureReason;
    ...
    setAccount(id: string, name?: string): void {
        ...
        if (name)
            this.setRuntimeAttribute(ATTR.ACCOUNT_NAME, name);
    }
    clearAccount(): void {
    ...
    reportError(error: unknown, opts?: {
        ...
        if (opts?.fingerprint)
            attrs[ATTR.ERROR_FINGERPRINT] = opts.fingerprint;
        if (opts?.category)
            attrs[ATTR.ERROR_CATEGORY] = opts.category;

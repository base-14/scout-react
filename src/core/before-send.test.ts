...
describe('applyBeforeSend', () => {
    ...
    it('passes type and name into the callback for filtering', () => {
        ...
        expect(cb).toHaveBeenCalledWith(expect.objectContaining({ type: 'log', name: 'login', 'enduser.id': 'u1' }));
    });
    ...
    it('preserves severity and message for logs', () => {
        const cb = vi.fn((e) => e);
        const result = applyBeforeSend(cb, 'log', 'log', { ok: true }, {
            severity: 'WARN',
            message: 'retrying',
        });
    ...
    it('allows the callback to override severity and message', () => {
        ...
        const result = applyBeforeSend(cb, 'log', 'log', {}, {
            severity: 'WARN',
            message: 'old',
        });

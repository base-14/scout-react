...
export function resourceTimingAttributes(entry: PerformanceResourceTiming): Attributes {
    ...
    phase(ATTR.HTTP_PHASE_REDIRECT_START_MS, ATTR.HTTP_PHASE_REDIRECT_DURATION_MS, entry.redirectStart, entry.redirectEnd);
    phase(ATTR.HTTP_PHASE_WORKER_START_MS, ATTR.HTTP_PHASE_WORKER_DURATION_MS, entry.workerStart ?? 0, entry.fetchStart);
    phase(ATTR.HTTP_PHASE_DNS_START_MS, ATTR.HTTP_PHASE_DNS_DURATION_MS, entry.domainLookupStart, entry.domainLookupEnd);
    phase(ATTR.HTTP_PHASE_CONNECT_START_MS, ATTR.HTTP_PHASE_CONNECT_DURATION_MS, entry.connectStart, entry.connectEnd);
    ...
    phase(ATTR.HTTP_PHASE_FIRST_BYTE_START_MS, ATTR.HTTP_PHASE_FIRST_BYTE_DURATION_MS, entry.requestStart, entry.responseStart);
    phase(ATTR.HTTP_PHASE_DOWNLOAD_START_MS, ATTR.HTTP_PHASE_DOWNLOAD_DURATION_MS, entry.responseStart, entry.responseEnd);
    ...
    const dt = (entry as PerformanceResourceTiming & {
        deliveryType?: string;
    })
        .deliveryType;
    if (dt)
        out[ATTR.HTTP_DELIVERY_TYPE] = dt;
    const rbs = (entry as PerformanceResourceTiming & {
        renderBlockingStatus?: string;
    })
        .renderBlockingStatus;
    if (rbs)
        out[ATTR.HTTP_RENDER_BLOCKING_STATUS] = rbs;
    if (entry.initiatorType)
        out[ATTR.HTTP_RESOURCE_TYPE] = entry.initiatorType;
    return out;
}

...
export function installFrustrationTracker(scout: Scout): () => void {
    ...
    const onClick = (e: MouseEvent) => {
        try {
            ...
            while (recent.length && now - recent[0]!.t > 1000)
                recent.shift();
            recent.push(row);
            ...
            setTimeout(() => {
                ...
                if (isRage)
                    frustrations.push('rage_click');
                const erroredNearby = Math.abs(performance.now() - lastErrorAt) < 100 ||
                    Math.abs(lastErrorAt - row.t) < 100;
                if (erroredNearby)
                    frustrations.push('error_click');

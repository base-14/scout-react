...
function describeElement(el: HTMLElement): {
    ...
    if (scoutAction)
        return { description: scoutAction, source: 'custom_attribute' };
    const aria = el.getAttribute?.('aria-label');
    if (aria)
        return { description: aria, source: 'standard_attribute' };
    const id = el.id;
    if (id)
        return { description: `#${id}`, source: 'standard_attribute' };
    ...
    if (text)
        return { description: text, source: 'text_content' };
    const cls = el.className && typeof el.className === 'string' ? el.className.split(' ')[0] : '';
    if (cls)
        return { description: `.${cls}`, source: 'standard_attribute' };

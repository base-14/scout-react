...
export function ScoutProvider({ config, children, fallback }: ScoutProviderProps) {
    ...
    const stable = useMemo(() => config, [config.serviceName, config.endpoint, config.serviceVersion, config.environment]);
    useEffect(() => {
        if (initRef.current)
            return;
    ...
    return (<ScoutContext.Provider value={{ ready: Scout.isInitialized }}>

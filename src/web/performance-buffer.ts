...
export function claimResourceEntry(
  url: string,
  fetchStart: number,
  ...
  for (const e of entries) {
    ...
    if (r.startTime + 2 < fetchStart) continue;
    if (consumed.has(key(r.name, r.startTime))) continue;
  ...
  if (best) {
    ...
    if (consumed.size > CLAIM_KEY_MAX) {
      ...
      consumed.clear();
      for (const k of arr.slice(arr.length / 2)) consumed.add(k);
    }
  }
  return best;
}

...
export class WebPlatform implements PlatformAdapter {
  ...
  async collectResourceAttributes(): Promise<Record<string, string | number | boolean>> {
    ...
    if (ua?.platform) attrs[ATTR.OS_NAME] = String(ua.platform);
    else if (nav.platform) attrs[ATTR.OS_NAME] = String(nav.platform);
    if (nav.userAgent) attrs['browser.user_agent'] = String(nav.userAgent);
    if (nav.language) attrs['browser.language'] = String(nav.language);
    const screen = (globalThis as any).screen;
    if (screen?.width) attrs['screen.width'] = Number(screen.width);
    if (screen?.height) attrs['screen.height'] = Number(screen.height);
  ...
  getConnectionType(): string {
    ...
    if (c?.effectiveType) return String(c.effectiveType);
